...
def get_labels(langue):
    ...
    if langue and langue.strip().lower().startswith("engl"):
        return {
            ...
            "years_suffix": "year(s)",
            "section_subscription": "PRODUCTS & SERVICES WITH SUBSCRIPTION",
            "section_onetime": "SERVICES & ONE-TIME PRODUCTS",
            "section_subscription_optional": "OPTIONS - PRODUCTS & SERVICES WITH SUBSCRIPTION",
            "section_onetime_optional": "OPTIONS - SERVICES & ONE-TIME PRODUCTS",
    ...
    return {
        ...
        "years_suffix": "an(s)",
        "section_subscription": "SERVICES & PRODUITS AVEC ABONNEMENT",
        ...
        "option_note": "Les lignes « Options » ne sont pas incluses dans le total ci-dessous ; elles seront facturées séparément si le client les confirme.",
        "recap_title": "RÉCAPITULATIF PLURIANNUEL CLIENT",
        "recap_col_label": "Types de coût",
        ...
        "year_label": "Année",
        "net_total": "Total net (hors option)",
        "vat": "TVA",
...
def generate_and_upload(item_id):
    ...
    # The engagement duration lives on each product line (already set when the
    # product was picked from the catalogue), not on the quote itself — so the
    # multi-year recap spans the longest commitment among the firm recurring
    # lines, rather than relying on a separate (and easy to forget) quote-level field.
    recap_years = max([l["duration"] for l in firm_recurring], default=1)

    years_recap = []
    for y in range(1, recap_years + 1):
    ...
    with PdfPages(buf) as pdf:
        ...
        def draw_table(y_top, title, rows_wrapped, row_lines, headers, col_widths, text_cols=2):
            ...
            needed = 0.022 + table_h_frac + 0.03  # title bar + table + bottom margin

            # If this table wouldn't fit on the remaining space of the current
            # page, start a fresh page for it instead of letting it run off
            # (and get silently clipped) or overlap the next section.
            if y_top - needed < 0.06:
                new_page()
                y_top = y
            ...
            for (r, c), cell in tbl.get_celld().items():
                ...
                cell.PAD = 0.03
                # Text-heavy leading columns (product/description/poste) stay
        ...
        y = draw_table(y, L["recap_title"], recap_rows, recap_lines, recap_headers, recap_widths, text_cols=1)
